...
{
    public partial class frm_moneda : DevExpress.XtraEditors.XtraForm
    {
        string id_usuario = Cls_Global.id_usuario;

        Cls_Grid_DevExpress_Mnt_1 Cls_Grid = new Cls_Grid_DevExpress_Mnt_1();
        List<EN_moneda.t_moneda> t_moneda = new List<EN_moneda.t_moneda>();
        DataTable dt_t_moneda_grid = new DataTable();
        DataTable dt_t_moneda_final = new DataTable();
        ...
        public frm_moneda()
        {
            ...
            labelControl1.Text = "Mantenimiento de monedas";
        ...
        private void inicio()
        {
            dt_t_moneda_grid = Cls_Grid.ListToTable(t_moneda);
            dt_t_moneda_final = Cls_Grid.ListToTable(t_moneda);

            gridControl1.DataSource = dt_t_moneda_grid;
    
            Cls_Grid.Load_Grid(gridControl1, gridView1, dt_t_moneda_grid);
        ...
        private void mnt_datos(string id_usuario)
        {
            ...
            try
            {
                ...
                t_moneda = dt_t_moneda_final.DataTableToList<EN_moneda.t_moneda>().ToList();
                var negocio = new LN_moneda();
                var parametro = new EN_moneda.proc_moneda_mnt();
                var retorno = new EN_moneda.proc_moneda_mnt_retorno();

                parametro.id_usuario = id_usuario;
                parametro.t_moneda = t_moneda;

                Cursor.Current = Cursors.WaitCursor;
                retorno = negocio.proc_moneda_mnt(parametro);
                ...
                {
                    retorno.t_moneda.ToList().ForEach(c => { c.id_usuario_inicia = ""; c.id_usuario_ultimo = "grabado"; });
                    dt_t_moneda_grid = Cls_Grid.ListToTable(retorno.t_moneda);
        ...
        private void gridControl1_EmbeddedNavigator_ButtonClick(object sender, NavigatorButtonClickEventArgs e)
        {
            ...
            {
                Cls_Grid.EmbeddedNavigator(gridView1, e, dt_t_moneda_grid);
            }
            ...
            if (e.Button.ButtonType == NavigatorButtonType.EndEdit)
            {
                ...
                dt_t_moneda_final = Cls_Grid.EmbeddedNavigator(gridView1, e, dt_t_moneda_grid);

                int count = dt_t_moneda_final.Rows.Count;
                if (count == 0)
                {
                    return;
                }
