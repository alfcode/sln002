...
{
    public partial class frm_unidad : DevExpress.XtraEditors.XtraForm
    {
        string id_usuario = Cls_Global.id_usuario;

        Cls_Grid_DevExpress_Mnt_1 Cls_Grid = new Cls_Grid_DevExpress_Mnt_1();
        List<EN_unidad.t_unidad> t_unidad = new List<EN_unidad.t_unidad>();
        DataTable dt_t_unidad_grid = new DataTable();
        DataTable dt_t_unidad_final = new DataTable();
        ...
        private void inicio()
        {
            dt_t_unidad_grid = Cls_Grid.ListToTable(t_unidad);
            dt_t_unidad_final = Cls_Grid.ListToTable(t_unidad);

            gridControl1.DataSource = dt_t_unidad_grid;
            Cls_Grid.Load_Grid(gridControl1, gridView1, dt_t_unidad_grid);
        }
        ...
        private void mnt_datos(string id_usuario)
        {
            ...
            try
            {
                ...
                t_unidad = dt_t_unidad_final.DataTableToList<EN_unidad.t_unidad>().ToList();
                var negocio = new LN_unidad();
                var parametro = new EN_unidad.proc_unidad_mnt();
                var retorno = new EN_unidad.proc_unidad_mnt_retorno();

                parametro.id_usuario = id_usuario;
                parametro.t_unidad = t_unidad;

                retorno = negocio.proc_unidad_mnt(parametro);
                ...
                {
                    retorno.t_unidad.ToList().ForEach(c => { c.id_usuario_inicia = ""; c.id_usuario_ultimo = "grabado"; });
                    dt_t_unidad_grid = Cls_Grid.ListToTable(retorno.t_unidad);
        ...
        private void gridControl1_EmbeddedNavigator_ButtonClick(object sender, NavigatorButtonClickEventArgs e)
        {
            ...
            {
                Cls_Grid.EmbeddedNavigator(gridView1, e, dt_t_unidad_grid);
            }
            ...
            if (e.Button.ButtonType == NavigatorButtonType.EndEdit)
            {
                ...
                dt_t_unidad_final = Cls_Grid.EmbeddedNavigator(gridView1, e, dt_t_unidad_grid);

                int count = dt_t_unidad_final.Rows.Count;
                if (count == 0)
                {
                    return;
                }
