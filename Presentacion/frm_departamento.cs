...
{
    public partial class frm_departamento : DevExpress.XtraEditors.XtraForm
    {
        string id_usuario = Cls_Global.id_usuario;

        Cls_Grid_DevExpress_Mnt_1 Cls_Grid = new Cls_Grid_DevExpress_Mnt_1();
        List<EN_departamento.t_departamento> t_departamento = new List<EN_departamento.t_departamento>();
        DataTable dt_t_departamento_grid = new DataTable();
        DataTable dt_t_departamento_final = new DataTable();
        ...
        public frm_departamento()
        {
            ...
            labelControl1.Text = "Mantenimiento de departamentos";
        ...
        private void inicio()
        {
            dt_t_departamento_grid = Cls_Grid.ListToTable(t_departamento);
            dt_t_departamento_final = Cls_Grid.ListToTable(t_departamento);

            gridControl1.DataSource = dt_t_departamento_grid;
     
            Cls_Grid.Load_Grid(gridControl1, gridView1, dt_t_departamento_grid);
        ...
        private void mnt_datos(string id_usuario)
        {
            ...
            try
            {
                ...
                t_departamento = dt_t_departamento_final.DataTableToList<EN_departamento.t_departamento>().ToList();
                var negocio = new LN_departamento();
                var parametro = new EN_departamento.proc_departamento_mnt();
                var retorno = new EN_departamento.proc_departamento_mnt_retorno();

                parametro.id_usuario = id_usuario;
                parametro.t_departamento = t_departamento;

                Cursor.Current = Cursors.WaitCursor;
                retorno = negocio.proc_departamento_mnt(parametro);
                ...
                {
                    retorno.t_departamento.ToList().ForEach(c => { c.id_usuario_inicia = ""; c.id_usuario_ultimo = "grabado"; });
                    dt_t_departamento_grid = Cls_Grid.ListToTable(retorno.t_departamento);
        ...
        private void gridControl1_EmbeddedNavigator_ButtonClick(object sender, NavigatorButtonClickEventArgs e)
        {
            ...
            {
                Cls_Grid.EmbeddedNavigator(gridView1, e, dt_t_departamento_grid);
            }
            ...
            if (e.Button.ButtonType == NavigatorButtonType.EndEdit)
            {
                ...
                dt_t_departamento_final = Cls_Grid.EmbeddedNavigator(gridView1, e, dt_t_departamento_grid);

                int count = dt_t_departamento_final.Rows.Count;
                if (count == 0)
                {
                    return;
                }
